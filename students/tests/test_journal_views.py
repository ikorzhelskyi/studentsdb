...
class JuornalViewsTest(TestCase):
    ...
    def test_get_pk_month(self):
        response = self.client.get(self.url_pk, {'month':'2020-02-01'})

        # do we have OK status from the server?
        self.assertEqual(response.status_code, 200)
        self.assertEqual(len(response.context['students']), 1)
        self.assertEqual(response.context['cur_month'], '2020-02-01')
        self.assertEqual(response.context['prev_month'], '2020-01-01')
        self.assertEqual(response.context['next_month'], '2020-03-01')
